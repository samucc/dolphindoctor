...
class BasicSettingForm(BaseForm):
    SITE_URL = forms.URLField(
        ...
        help_text="eg: http://dolphindoctor.abc.com:8080"
    )
...
class EmailSettingForm(BaseForm):
    EMAIL_HOST = forms.CharField(
        max_length=1024, label=_("SMTP host"), initial='smtp.dolphindoctor.org'
    )
    EMAIL_PORT = forms.CharField(max_length=5, label=_("SMTP port"), initial=25)
    EMAIL_HOST_USER = forms.CharField(
        max_length=128, label=_("SMTP user"), initial='noreply@dolphindoctor.org'
    )
...
class LDAPSettingForm(BaseForm):
    ...
    AUTH_LDAP_USER_ATTR_MAP = FormDictField(
        ...
        help_text=_(
            "User attr map present how to map LDAP user attr to dolphindoctor, "
            "username,name,email is dolphindoctor attr"
        ),
    )
...
class EmailContentSettingForm(BaseForm):
    ...
    EMAIL_CUSTOM_USER_CREATED_SIGNATURE = forms.CharField(
        ...
        help_text=_("Tips: Email signature (eg:dolphindoctor)")
    )
