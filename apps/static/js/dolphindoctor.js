//dolphindoctor 自定义js 2015-01-29
...
dolphindoctor.selected = {};
dolphindoctor.language = {
...
dolphindoctor.initDataTable = function (options) {
  // options = {
  //    ele *: $('#dataTable_id'),
  ...
  var table = ele.DataTable({
    ...
    table.on('select', function(e, dt, type, indexes) {
        ...
        dolphindoctor.selected[$node.find('input.ipt_check').prop('id')] = true
    }).on('deselect', function(e, dt, type, indexes) {
        var $node = table[ type ]( indexes ).nodes().to$();
        $node.find('input.ipt_check').prop('checked', false);
        dolphindoctor.selected[$node.find('input.ipt_check').prop('id')] = false
    }).on('draw', function(){
...
dolphindoctor.initStaticTable = function (selector) {
    $(selector).DataTable({
        "searching": false,
        "bInfo": false,
        "paging": false,
...
dolphindoctor.initServerSideDataTable = function (options) {
  // options = {
  //    ele *: $('#dataTable_id'),
  //    ajax_url *: '{% url 'users:user-list-api' %}',
  //    columns *: [{data: ''}, ....],
